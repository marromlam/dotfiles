...
local spoonInstallLoaded = hs.loadSpoon('SpoonInstall')
if not spoonInstallLoaded then
  hs.alert.show('SpoonInstall not found; spoon features disabled')
end

-- Clock {{{

if spoonInstallLoaded then
...
local function update_wallpaper()
  if not spoonInstallLoaded then
    hs.alert.show('SpoonInstall missing')
    return
  end

  spoon.SpoonInstall:andUse(
    'BingDaily',
    { screen = hs.screen.allScreens(), runAt = '11:41' }
  )
  if spoon.BingDaily then
    spoon.BingDaily:start()
    hs.alert.show('Wallpaper updated')
  else
    hs.alert.show('BingDaily spoon unavailable')
  end
end
...
function ssidChangedCallback()
  ...
  if SSID == nil then SSID = 'Disconnected' end
...
function displayMicMuteStatus()
  ...
  local currentAudioInputObject =
    hs.audiodevice.findInputByUID(currentAudioInput.uid)
  muted = currentAudioInputObject:inputMuted()
  if muted then
    micMuteStatusMenu:setIcon(os.getenv('HOME') .. '/.hammerspoon/muted.png')
    micMuteStatusLineColor =
      { ['red'] = 1, ['blue'] = 1, ['green'] = 1, ['alpha'] = 0.0 }
  else
    micMuteStatusMenu:setIcon(os.getenv('HOME') .. '/.hammerspoon/unmuted.png')
    micMuteStatusLineColor =
      { ['red'] = 1, ['blue'] = 0, ['green'] = 0, ['alpha'] = 0.5 }
  end
  if micMuteStatusLine then micMuteStatusLine:delete() end
  max = hs.screen.primaryScreen():fullFrame()
  micMuteStatusLine =
    hs.drawing.rectangle(hs.geometry.rect(max.x, max.y, max.w, max.h))
...
-- Complements Amethyst: easy "rescue" positions for float dialogs/apps.
hs.hotkey.bind(
  hyper_shift,
  'Left',
  function() snapFocusedWindow({ x = 0.0, y = 0.0, w = 0.5, h = 1.0 }) end
)
hs.hotkey.bind(
  hyper_shift,
  'Right',
  function() snapFocusedWindow({ x = 0.5, y = 0.0, w = 0.5, h = 1.0 }) end
)
hs.hotkey.bind(
  hyper_shift,
  'Up',
  function() snapFocusedWindow({ x = 0.0, y = 0.0, w = 1.0, h = 1.0 }) end
)
hs.hotkey.bind(hyper_shift, 'Down', centerFocusedWindow)

-- Quick utility shortcuts
hs.hotkey.bind(
  hyper,
  'A',
  function() hs.application.launchOrFocus('Amethyst') end
)
hs.hotkey.bind(hyper_shift, 'R', function() hs.reload() end)
