...
sed -i "19s/.*/    sha256 cellar: :any, x86_64_linux: \"cd94b6bc2189df7861c2c32c480f777984865dbab4107f493188feda5a05b80d\"/" $HOMEBREW_PREFIX/Homebrew/Library/Taps/homebrew/homebrew-core/Formula/gcc@5.rb
...
if [[ -d "$HOMEBREW_PREFIX/Cellar/lemonade" ]]; then
  ...
  mkdir $HOMEBREW_PREFIX/Cellar/lemonade
  tar xf lemonade.tar.gz -C $HOMEBREW_PREFIX/Cellar/lemonade
  ln -sf $HOMEBREW_PREFIX/Cellar/lemonade/lemonade $HOMEBREW_PREFIX/bin
  rm lemonade.tar.gz
fi
...
if [[ -d "$HOMEBREW_PREFIX/Cellar/termpdf.py" ]]; then
  ...
  mkdir $HOMEBREW_PREFIX/Cellar/termpdf.py
  git clone https://github.com/dsanson/termpdf.py.git $HOMEBREW_PREFIX/Cellar/termpdf.py
  cd $HOMEBREW_PREFIX/Cellar/termpdf.py
